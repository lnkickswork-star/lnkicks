...
- **PostgreSQL connection strings** (`DATABASE_URL`, `DIRECT_URL`) — production DB. Get from Supabase / Neon / Railway / Render dashboard. MUST include `?sslmode=require`.
- Payment gateway keys (Stripe, Razorpay)
- JWT secrets (auto-generated by `setup-env-vars.sh`)
- SMTP credentials
