#!/usr/bin/env bash
# =============================================================================
# backup-current.sh — Snapshot the current production deployment for rollback
# =============================================================================
#
# Called by the GitHub Actions deploy workflow BEFORE uploading new code.
# Creates a timestamped copy of the APP_ROOT directory under releases/.
# Also prunes old backups to keep the disk from filling up (keeps last 10).
#
# Usage (invoked over SSH from GitHub Actions):
#   bash backup-current.sh <APP_ROOT> <GIT_SHA> <RELEASES_DIR>
#
# Args:
#   APP_ROOT     — absolute path to the app root (e.g. /home/aqualit1/lnkicks)
#                  Code lives DIRECTLY here (no /current subdir).
#   GIT_SHA      — full git commit SHA being deployed (for traceability)
#   RELEASES_DIR — absolute path to backups directory
#                  (e.g. /home/aqualit1/lnkicks-releases — OUTSIDE app root
#                  so it isn't touched by rsync --delete during deploy)
#
# Layout produced on the server:
#   /home/aqualit1/lnkicks/              ← APP_ROOT (live code, deployed here)
#   /home/aqualit1/lnkicks-releases/     ← RELEASES_DIR (backups)
#     ├── 20250101-120000-abc1234/       ← timestamped backup
#     ├── 20250101-110000-def5678/
#     ├── latest → (symlink to most recent)
#     └── failed-20250101-130000-xyz9999/  ← (created by rollback.sh if needed)
# =============================================================================
set -euo pipefail

APP_ROOT="${1:?APP_ROOT argument required}"
GIT_SHA="${2:?GIT_SHA argument required}"
RELEASES_DIR="${3:?RELEASES_DIR argument required}"

# If no app exists yet (first-ever deploy), nothing to back up.
if [[ ! -d "$APP_ROOT" ]]; then
  echo "[backup-current] No existing app at $APP_ROOT — first deployment, skipping backup."
  exit 0
fi

# If app exists but has no .next/ (never deployed successfully), skip backup.
if [[ ! -d "$APP_ROOT/.next" ]]; then
  echo "[backup-current] App exists but no .next/ directory — first deployment, skipping backup."
  exit 0
fi

# Timestamp + short SHA for the backup directory name.
TIMESTAMP=$(date -u '+%Y%m%d-%H%M%S')
SHORT_SHA=$(echo "$GIT_SHA" | cut -c1-7)
BACKUP_DIR="$RELEASES_DIR/$TIMESTAMP-$SHORT_SHA"

echo "[backup-current] Creating releases directory: $RELEASES_DIR"
mkdir -p "$RELEASES_DIR"

echo "[backup-current] Backing up $APP_ROOT → $BACKUP_DIR"

# Copy ONLY the app code, NOT the node_modules (huge) or tmp (restart state)
# or scripts (managed separately) — these would bloat backups.
# We use cp -a to preserve permissions, ownership, and symlinks.
mkdir -p "$BACKUP_DIR"

# Copy each critical path individually so we can skip large/regenerable dirs.
for item in .next public cpanel package.json package-lock.json next.config.js; do
  if [[ -e "$APP_ROOT/$item" ]]; then
    cp -a "$APP_ROOT/$item" "$BACKUP_DIR/"
  fi
done

# Write a manifest with deploy metadata for debugging / auditing.
cat > "$BACKUP_DIR/.deploy-manifest.json" <<EOF
{
  "backed_up_at": "$(date -u '+%Y-%m-%dT%H:%M:%SZ')",
  "git_sha": "$GIT_SHA",
  "short_sha": "$SHORT_SHA",
  "backed_up_by": "${USER:-unknown}",
  "host": "$(hostname -f 2>/dev/null || hostname)",
  "app_root": "$APP_ROOT"
}
EOF

# Update the "latest backup" pointer symlink — used by rollback.sh to find
# the most recent good version without parsing directory names.
ln -sfn "$BACKUP_DIR" "$RELEASES_DIR/latest"

echo "[backup-current] ✅ Backup created: $BACKUP_DIR"
echo "[backup-current] 'latest' pointer updated → $RELEASES_DIR/latest"
echo "[backup-current] Backup size: $(du -sh "$BACKUP_DIR" | cut -f1)"

# Prune: keep the 10 most recent backups, delete older ones.
# This prevents the disk from slowly filling up over months of deploys.
echo "[backup-current] Pruning old backups (keeping last 10)..."
cd "$RELEASES_DIR" || { echo "[backup-current] ERROR: cannot cd to $RELEASES_DIR" >&2; exit 1; }
# Use find (not ls) — handles filenames with dashes/spaces correctly.
# List directories newest-first, skip the first 10, delete the rest.
find . -maxdepth 1 -mindepth 1 -type d -not -name '.*' -printf '%T@ %p\n' 2>/dev/null \
  | sort -rn \
  | tail -n +11 \
  | while read -r _ old_dir; do
      old_dir="${old_dir#./}"
      echo "[backup-current] Removing old backup: $old_dir"
      # ${RELEASES_DIR:?} ensures this never expands to / (defensive).
      rm -rf -- "${RELEASES_DIR:?}/${old_dir}"
    done

echo "[backup-current] Done. Current backups:"
find "$RELEASES_DIR" -maxdepth 1 -mindepth 1 -type d -not -name '.*' -printf '%T@ %p\n' 2>/dev/null \
  | sort -rn \
  | head -n 10 \
  | while read -r _ d; do echo "  $d"; done || true
